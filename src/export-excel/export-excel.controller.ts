...
export class ExportExcelController {
  ...
  async downloadWebinars(
    @Body() body:{filters: WebinarFilterDTO, columns: string[], fieldName: string},
    @Id() adminId: string,
    @Query('limit') limit: string,
    @Res() res: Response,
  ): Promise<void> {
    try {
      ...
      if(!adminId)
        throw new BadRequestException('Admin ID is required to download webinars Excel file.');
      const filePath = await this.exportExcelService.generateExcelForWebinar(parseInt(limit) || 100, body.columns, body.filters, adminId);
