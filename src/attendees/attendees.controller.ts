...
export class AttendeesController {
  ...
  async getAttendees(
    ...
    if(!query?.isAttended) throw new NotAcceptableException('isAttended search query is required')
  ...
  async addAttendees(
    @Id() adminId: string,
    @Body() body: { data: [CreateAttendeeDto]; webinarId: string; isAttended: boolean },
  ): Promise<any> {
    console.log(body)
